		...
		<Columns>
			...
			<asp:TemplateField>
				<ItemTemplate>
					&nbsp;<asp:ImageButton id="imgDelPlan" runat="server" Text="Delete" SkinID="ExchangeDelete"
						CommandName="DeleteItem" CommandArgument='<%# Eval("SfBUserPlanId") %>' 
			...
			<asp:TemplateField>
				<ItemTemplate>
                        ...
                        Text="Restamp all Skype for Business users with this plan" CssClass="Button1"  CommandName="RestampItem" CommandArgument='<%# Eval("SfBUserPlanId") %>' OnClientClick="if (confirm('Restamp SfB user with this plan.\n\nAre you sure you want to restamp the SfB users ?')) ShowProgressDialog('Stamping SfB users, this might take a while ...'); else return false;"/>
				</ItemTemplate>
			</asp:TemplateField>
		</Columns>
	</asp:GridView>
